...
import { getValidGitHubToken } from './github/tokens.js';
...
export async function getValidToken(provider, connectionType, { userId = null, organizationId = null, forceRefresh = false } = {}) {
    ...
    if (provider === 'github') {
        if (!record) return { token: null, metadata: {}, id: null };
    ...
    } else if (['youtube', 'google', 'google_workspace'].includes(provider)) {
        const metadata = table === 'integrations' ? record.settings : record.metadata;
        const refresh_token = record.refresh_token || metadata?.refresh_token;
        const expires_at = metadata?.expires_at || ((metadata?.created_at || 0) + (metadata?.expires_in || 0));

        const now = Math.floor(Date.now() / 1000);
        const buffer = 300; // 5 minutes
        const isExpired = expires_at && now >= (expires_at - buffer);

        if (refresh_token && (isExpired || forceRefresh)) {
            console.log(`[AUTH-UTIL] ${provider} token for ${record.id} is ${isExpired ? 'expired' : 'being force refreshed'}. Refreshing...`);
            try {
                ...
                if (data.access_token) {
                    ...
                    if (table === 'integrations') {
                        await supabase.from('integrations').update({ settings: updatedMetadata }).eq('id', record.id);
                    } else {
                    ...
                    console.error(`[AUTH-UTIL] ${provider} refresh failed: No access token in response`, data);
                }
            } catch (e) {
                console.error(`[AUTH-UTIL] ${provider} refresh failed:`, e.message);
            }
        }
        ...
        if (refresh_token && (isExpired || forceRefresh)) {
            console.log(`[AUTH-UTIL] Streamlabs token for ${record.id} is ${isExpired ? 'expired' : 'being force refreshed'}. Refreshing...`);
            try {
                ...
                if (data.access_token) {
                    ...
                    console.error('[AUTH-UTIL] Streamlabs refresh failed: No access token in response', data);
                }
            } catch (e) {
                console.error('[AUTH-UTIL] Streamlabs refresh failed:', e.message);
            }
        }
        ...
        if (refresh_token && (isExpired || forceRefresh)) {
            console.log(`[AUTH-UTIL] TikTok token for ${record.id} is ${isExpired ? 'expired' : 'being force refreshed'}. Refreshing...`);
            try {
                ...
                if (data.access_token) {
                    ...
                    console.error('[AUTH-UTIL] TikTok refresh failed: No access token in response', data);
                }
            } catch (e) {
                console.error('[AUTH-UTIL] TikTok refresh failed:', e.message);
            }
        }
